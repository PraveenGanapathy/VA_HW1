...
def visualize_tfidf_features(X, vectorizer, kmeans, filenames, optimal_k):
    ...
    for i in range(optimal_k):
        ...
        term_scores = [kmeans.cluster_centers_[i]
                       [order_centroids[i, j]] for j in range(10)]
...
def process_document(args):
    ...
    entity_counts = {
        ...
        'LOCATION': len(entities['LOCATION'] + entities['GPE']),
        'ORGANIZATION': len(entities['ORGANIZATION']),
        # Add date detection
        'DATE': len(re.findall(r'\d{4}-\d{2}-\d{2}', doc)),
        'FACILITY': len(entities['FACILITY'])
    }
    return filename, preprocessed, entities, entity_counts
...
def perform_mds(matrix):
    ...
    return MDS(n_components=2, random_state=42).fit_transform(scaled_matrix)
...
def main():
    try:
        ...
        for filename, preprocessed, entities, entity_counts in results:
            processed_data[filename] = {
                'preprocessed': preprocessed, 'entities': entities, 'entity_counts': entity_counts}
            preprocessed_documents.append(preprocessed)
            document_entities[filename] = entities
            document_entity_counts.append(entity_counts)
        entity_types = ['PERSON', 'LOCATION',
                        'ORGANIZATION', 'DATE', 'FACILITY']
        document_entity_matrix = np.array([
            [counts[et] for et in entity_types]
            for counts in document_entity_counts  # Use pre-collected counts
        ])

        print("Entity types:", entity_types)
        print("Matrix shape:", document_entity_matrix.shape)
        print("Sample row:", document_entity_matrix[0])

        mds_coordinates = perform_mds(document_entity_matrix)

        # Create TF-IDF matrix
        vectorizer = TfidfVectorizer(
            max_df=0.8, min_df=1, stop_words='english')
        ...
        plt.figure(figsize=(10, 8))
        scatter = plt.scatter(
            mds_coordinates[:, 0], mds_coordinates[:, 1], c=kmeans_labels, cmap='plasma')
        plt.title('Document Clusters Visualization (MDS)')
        ...
        for i, filename in enumerate(filenames):
            plt.annotate(
                filename, (mds_coordinates[i, 0], mds_coordinates[i, 1]), fontsize=8)

        plt.savefig('document_clusters_mds.png', dpi=300)
        ...
        try:
            biclustering = SpectralBiclustering(
                n_clusters=optimal_k, random_state=42)
            biclustering.fit(X)
            biclustering_labels = biclustering.row_labels_
        except Exception as e:
            print(f"Error in biclustering: {e}")
            biclustering_labels = np.zeros(
                len(documents), dtype=int)  # Default to all zeros
        ...
        top_terms_per_cluster = visualize_tfidf_features(
            X, vectorizer, kmeans, filenames, optimal_k)
        ...
        document_features = {
            ...
            'mds_coordinates': mds_coordinates.tolist()
        }
        # Add to document_features.json
        document_features['mds_coordinates'] = [
            {'x': coord[0], 'y': coord[1], 'docId': filenames[i],
