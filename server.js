...
app.post('/api/cluster-documents', (req, res) => {
    ...
    try {
        // Read the pre-generated JSON files from the data directory
        const clusteringResultsPath = path.join(rootDir, 'data', 'clustering_results.json');
        const documentFeaturesPath = path.join(rootDir, 'data', 'document_features.json');
        
        // Check if files exist
        if (!fs.existsSync(clusteringResultsPath) || !fs.existsSync(documentFeaturesPath)) {
            return res.status(404).json({ 
                error: 'Pre-generated data files not found. Please check the data directory.' 
            });
        }
        
        // Read the clustering results from the files
        const clusteringResults = JSON.parse(fs.readFileSync(clusteringResultsPath, 'utf8'));
        const documentFeatures = JSON.parse(fs.readFileSync(documentFeaturesPath, 'utf8'));
        ...
        res.status(500).json({ error: 'Failed to read clustering results', details: error.message });
    }
});
